...
MODES = ["Query Files", "Query Files (no stream)", "Search Files", "LLM Chat (no context from files)"]
    ...
    def _chat(self, message: str, history: list[list[str]], mode: str, *_: Any) -> Any:
        ...
        match mode:
          ...
          # Use only the selected file for the query
                ...
                logger.debug("chat: messages: %s, \n context_filter: %s", all_messages, context_filter )
                logger.debug("chat: response: %s", query_response.response)

                full_response: str = query_response.response
                if query_response.sources:
                    full_response += SOURCES_SEPARATOR
                    cur_sources = Source.curate_sources(query_response.sources)
                    sources_text = "\n\n\n".join(
                        f"{index}. {source.file} (page {source.page})"
                        for index, source in enumerate(cur_sources, start=1)
                    )
